...
class MultiFrameVDModel(BaseModel):
    def __init__(self, opt):
        ...
        with get_accelerator().device(0):
            flops, macs, params = get_model_profile(model=self.net,
                                                    input_shape=(1, 3, 3, 720, 1280),
                                                    args=None, kwargs=None,
                                                    print_profile=True, detailed=True,
                                                    module_depth=-1, top_modules=1,
                                                    warm_up=10, as_string=True,
    ...
    def optimize_parameters(self, current_iter):
        ...
        # self.clean, self.moire: (B, T, C, H, W)
        ...
        if self.cri_self_recon:
            gt_flat = self.gt_frames.view(B * T, C, H, W)
            l_self_recon = self.cri_self_recon(
                self.lq_recon.view(B * T, C, H, W),
                _match_stats(self.lq.view(B * T, C, H, W), gt_flat)
            )
            l_total += l_self_recon
            loss_dict['l_self_recon'] = l_self_recon

        if self.cri_mid_pix:
            l_mid_pixel = self.cri_mid_pix(
                self.clean.view(B * T, C, H, W),
                self.gt_frames.view(B * T, C, H, W)
            )
            l_total += l_mid_pixel
            loss_dict['l_mid_pixel'] = l_mid_pixel

        if self.cri_mid_perceptual:
            l_mid_percep, l_mid_style = self.cri_mid_perceptual(
                self.clean.view(B * T, C, H, W),
                self.gt_frames.view(B * T, C, H, W)
            )
            if l_mid_percep is not None:
                l_total += l_mid_percep
                loss_dict['l_mid_percep'] = l_mid_percep
            if l_mid_style is not None:
                l_total += l_mid_style
                loss_dict['l_mid_style'] = l_mid_style

        if self.cri_decomp_contrastive:
            adain_lq = _match_stats(
                self.lq.view(B * T, C, H, W),
                self.gt_frames.view(B * T, C, H, W).detach()
            )
            l_decomp_contrastive = self.cri_decomp_contrastive(
                self.moire, adain_lq.view(B, T, C, H, W), self.gt_frames
            )
            l_total += l_decomp_contrastive
            loss_dict['l_decomp_contrastive'] = l_decomp_contrastive
    ...
    def test(self):
        ...
        if hasattr(self, 'net_ema'):
            self.net_ema.eval()
            with torch.no_grad():
                self.output, self.clean, self.moire = self.net_ema(self.lq)
